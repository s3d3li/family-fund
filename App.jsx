...
export default function App() {
  ...
  function addExpense() {
    ...
    const newExp = { id:data.nextExpenseId, title:expForm.title, amount:parseFloat(expForm.amount), date:expForm.date, category:expForm.category, note:expForm.note };
        ...
        {tab==="expenses" && (
          <div>
            ...
            {data.expenses.length===0
              ...
              : data.expenses.map(e => (
                <div key={e.id} style={{ background:"#fff", borderRadius:16, padding:16, marginBottom:10, boxShadow:"0 2px 8px rgba(0,0,0,0.06)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ background:"#fff", borderRadius:"24px 24px 0 0", padding:24, width:"100%", maxHeight:"85vh", overflowY:"auto" }}>
            ...
            {modal==="expense" && (
              <div>
                ...
                <button onClick={addExpense} style={{ ...btnPrimary, background:"linear-gradient(135deg,#EF4444,#F97316)" }}>تسجيل المصروف</button>
              </div>
            )}
